...
def base_opts() -> dict:
    """Opzioni comuni a tutti i task (info + download) per evitare il 403."""
    return {
        "quiet": True,
        "no_warnings": True,
        # User-Agent realistico di un browser moderno
        "http_headers": {
            "User-Agent": (
        ...
        # Usa i cookie del browser installato (prova in ordine)
        # Cambia "chrome" con "firefox" / "edge" / "brave" se usi un altro browser
        "cookiesfrombrowser": ("chrome",),
        # Riprova automaticamente in caso di errori temporanei
        "retries": 5,
        "fragment_retries": 5,
    }
...
def recupera_info(url: str) -> dict:
    """Scarica i metadati del video senza effettuare il download."""
    ydl_opts = base_opts()
